...
<div class="card shadow-sm">
    <div class="card-header">
        <h5 class="mb-0 fw-semibold">Novo Pedido</h5>
    </div>
    <div class="card-body">
        <form method="post" action="/admin/pedidos/store">
            <?= \App\Core\Csrf::input() ?>
                ...
                <select class="form-select <?= isset($errors['cliente_id']) ? 'is-invalid' : '' ?>" 
                    ...
                    <?php foreach ($clientes as $cliente): ?>
                        <option value="<?= $this->e($cliente['id']) ?>" 
                                <?= (isset($old['cliente_id']) && $old['cliente_id'] == $cliente['id']) ? 'selected' : '' ?>>
                ...
                <select class="form-select <?= isset($errors['product_id']) ? 'is-invalid' : '' ?>" 
                    ...
                    <?php foreach ($products as $product): ?>
                        <option value="<?= $this->e($product['id']) ?>" 
                                ...
                                <?= (isset($old['product_id']) && $old['product_id'] == $product['id']) ? 'selected' : '' ?>>
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control <?= isset($errors['quantidade']) ? 'is-invalid' : '' ?>" 
                       ...
                       value="<?= $this->e($old['quantidade'] ?? '1') ?>" required>
                ...
                <select class="form-select <?= isset($errors['forma_pagamento_id']) ? 'is-invalid' : '' ?>" 
                    ...
                    <?php foreach ($formasPagamento as $fp): ?>
                        <option value="<?= $this->e($fp['id']) ?>" 
                                <?= (isset($old['forma_pagamento_id']) && $old['forma_pagamento_id'] == $fp['id']) ? 'selected' : '' ?>>
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="status" name="status">
                    ...
                    <option value="concluido" <?= (isset($old['status']) && $old['status'] == 'concluido') ? 'selected' : '' ?>>Concluído</option>
                    <option value="cancelado" <?= (isset($old['status']) && $old['status'] == 'cancelado') ? 'selected' : '' ?>>Cancelado</option>
                </select>
            </div>
